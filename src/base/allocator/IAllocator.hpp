...
#define CAL_NEW(allocator, ...) new (cal::NewPlaceholder(), (allocator).allocate(sizeof(__VA_ARGS__), alignof(__VA_ARGS__))) __VA_ARGS__
#define CAL_DEL(allocator, var) (allocator).deleteObject(var);

namespace cal { struct NewPlaceholder {}; }
inline void* operator new(size_t, cal::NewPlaceholder, void* where) { return where; }
inline void operator delete(void*, cal::NewPlaceholder, void*) { }
...
namespace cal {
    ...
    struct UniquePtr {
        ...
        template <typename... Args> static UniquePtr<T> create(IAllocator& allocator, Args&&... args) {
            return UniquePtr<T>(CAL_NEW(allocator, T) (static_cast<Args&&>(args)...), &allocator);
        }
