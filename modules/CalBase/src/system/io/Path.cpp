...
namespace cal {
    ...
    void Path::add(StringView value) {
        ...
        m_length = u32(string::copyString(span, value) - m_path);
    }


    void Path::add(StableHash hash) {
        Span span(m_path + m_length, m_path + MAX_PATH);
        m_length = u32(string::toCString(hash.getHashValue(), span) - m_path);
    }


    void Path::add(u64 value) {
        Span span(m_path + m_length, m_path + MAX_PATH);
        m_length = u32(string::toCString(value, span) - m_path);
    }
    ...
    bool Path::operator==(const char* rhs) const {
        return string::equalStrings(rhs, m_path);
    }
    ...
    bool Path::operator==(const Path& rhs) const {
        ASSERT(string::equalStrings(m_path, rhs.m_path) == (m_hash == rhs.m_hash));
    ...
    bool Path::isSame(StringView a, StringView b) {
        ...
        return string::equalStrings(a, b);
    }


    bool Path::replaceExtension(char* path, const char* ext)
    {
        char* end = path + string::stringLength(path);
        while (end > path && *end != '.')
        {
            --end;
        }
    ...
    bool Path::hasExtension(StringView filename, StringView ext) {
        return string::equalIStrings(getExtension(filename), ext);
    }
