...
namespace neo
{
    ...
    static const std::map<StringView, TokenType> s_idTypes{
        {"var", TokenType::kVar},
        {"val", TokenType::kVal},
        {"fun", TokenType::kFun},
    ...
    TokenType NToken::checkIdentifier(const StringView &c)
    {
        StringView rc = c.data();
    ...
    StringView NToken::typeString(TokenType tk)
    {
        int idx = (int)tk;
        if (idx < 0 || idx > 86)
        {
    ...
    String NToken::toString() const
    {
        return "{ " + String(s_typeStrings[(int)type]) + " : " + value + " }";
    }

    StringView NToken::typeString() const
    {
